...
const SapphireScene = forwardRef<SapphireSceneController, SapphireSceneProps>(({ mode = '3d' }, ref) => {
  ...
  // Keep mutable references to Three.js objects for direct manipulation by GSAP
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (gemstone) {
        // Gentle rotation overlay
        gemstone.rotation.y += 0.002;
  ...
  return (
    <div className="sapphire-visual-container webgl-mode" ref={containerRef}>
      <canvas ref={canvasRef} className="sapphire-canvas" aria-label="Interactive 3D Sapphire Crystal" />
    </div>
  );
});
